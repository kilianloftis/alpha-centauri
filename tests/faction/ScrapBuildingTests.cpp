// Player scrap of a constructed building: production.json kinds.building.scrap formula
// over listed mineral cost, paid as energy credits. Combat / probe / raze destruction
// does not refund.
...
ProductionConfig_t ScrapConfig(std::string formula)
{
    ProductionConfig_t config;
    ScrapKindConfig_t scrap;
    scrap.formula = std::move(formula);
    scrap.refundType = ScrapRefundType_t::EnergyCredits;
    config.kinds[ConstructableKind_t::Building].scrap = std::move(scrap);
    return config;
}
...
TEST_CASE("Scrap refund evaluates the kind's formula over listed mineral cost", "[building][scrap]")
{
    ...
    CHECK(halfQuote.refundType == ScrapRefundType_t::EnergyCredits);
          ...
          "[building][scrap]")
{
    ...
    REQUIRE(base.QuoteScrapBuilding("test_hurry_facility") == 10);
...
TEST_CASE("Production config groups scrap under kinds", "[building][scrap][config]")
{
    SECTION("shipping shape loads")
    {
        const TempConfigFile file("scrap_ok.json", R"json({
            ...
            "kinds": {
                "building": {
                    "scrap": { "formula": "floor(minerals / 4)", "refund_type": "energy_credits" }
                }
            }
        })json");
        const ProductionConfig_t config = ProductionConfigParser{}.ParseConfig(file.Path());
        REQUIRE(config.kinds.at(ConstructableKind_t::Building).scrap.has_value());
        const ScrapKindConfig_t& rScrap = *config.kinds.at(ConstructableKind_t::Building).scrap;
        CHECK(rScrap.formula == "floor(minerals / 4)");
        CHECK(rScrap.refundType == ScrapRefundType_t::EnergyCredits);
    }

    SECTION("omitting scrap on a kind leaves scrap disabled for it")
    {
        ...
        const ProductionConfig_t config = ProductionConfigParser{}.ParseConfig(file.Path());
        CHECK_FALSE(config.kinds.at(ConstructableKind_t::Building).scrap.has_value());
    }
    ...
    SECTION("a kind missing formula throws")
    {
        const TempConfigFile file("scrap_no_formula.json", R"({
            ...
            "retool_penalty_percent": 50,
            "prototype_surcharge_percent": 50,
            "kinds": { "building": { "scrap": { "refund_type": "energy_credits" } } }
        })");
        CHECK_THROWS_WITH(ProductionConfigParser{}.ParseConfig(file.Path()),
                          Catch::Matchers::ContainsSubstring("kinds.building.scrap")
    ...
    SECTION("a kind missing refund_type throws")
    {
        const TempConfigFile file("scrap_no_type.json", R"json({
            ...
            "retool_penalty_percent": 50,
            "prototype_surcharge_percent": 50,
            "kinds": { "building": { "scrap": { "formula": "floor(minerals / 2)" } } }
        })json");
        CHECK_THROWS_WITH(ProductionConfigParser{}.ParseConfig(file.Path()),
                          Catch::Matchers::ContainsSubstring("kinds.building.scrap")
    ...
    SECTION("an unknown refund_type throws")
    {
        const TempConfigFile file("scrap_bad_type.json", R"({
            ...
            "retool_penalty_percent": 50,
            "prototype_surcharge_percent": 50,
            "kinds": { "building": { "scrap": { "formula": "1", "refund_type": "gold" } } }
        })");
        CHECK_THROWS_WITH(ProductionConfigParser{}.ParseConfig(file.Path()),
                          Catch::Matchers::ContainsSubstring("refund_type"));
    }

    SECTION("secret_project cannot have a scrap block")
    {
        const TempConfigFile file("scrap_sp.json", R"json({
            ...
            "kinds": {
                "secret_project": {
                    "scrap": { "formula": "floor(minerals / 2)", "refund_type": "energy_credits" }
                }
            }
        })json");
        CHECK_THROWS_WITH(ProductionConfigParser{}.ParseConfig(file.Path()),
                          Catch::Matchers::ContainsSubstring("kinds.secret_project.scrap")
                              && Catch::Matchers::ContainsSubstring("cannot be scrapped"));
    }
}
