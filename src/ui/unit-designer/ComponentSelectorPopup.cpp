...
{
    const float padding = m_layout.height * Style().componentSelectorPopup.paddingRatio;
    const float entryHeight = m_layout.height * Style().componentSelectorPopup.entryHeightRatio;
    const float titleHeight = m_layout.height * Style().componentSelectorPopup.titleFontSizeRatio
        * Style().componentSelectorPopup.titleHeightMultiplier;
...
void ComponentSelectorPopup::Render(Graphics& rGraphics)
{
    rGraphics.DrawFilledRect(
        ...
        Style().componentSelectorPopup.backgroundColor
    );
    rGraphics.DrawRect(
        m_layout.x, m_layout.y, m_layout.width, m_layout.height,
        Style().componentSelectorPopup.borderColor,
        Style().componentSelectorPopup.borderWidth
    );

    const float padding = m_layout.height * Style().componentSelectorPopup.paddingRatio;
    const unsigned int titleSize = static_cast<unsigned int>(
        m_layout.height * Style().componentSelectorPopup.titleFontSizeRatio);
    const unsigned int entrySize = static_cast<unsigned int>(
        m_layout.height * Style().componentSelectorPopup.entryFontSizeRatio);
    ...
    rGraphics.DrawText(
        ...
        m_layout.y + padding,
        titleSize,
        Style().componentSelectorPopup.titleColor
    );
    ...
    for (size_t i = 0; i < m_components.size() && i < m_entryRects.size(); ++i)
    {
        ...
        rGraphics.DrawText(m_components[i]->name, rRect.x + padding, rRect.y, entrySize,
                           Style().componentSelectorPopup.entryColor);
    }
}
