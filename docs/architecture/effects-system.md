...
- **Values**:
  ...
  - Commerce: `CommerceRate` (PureMultiplier, Faction — Global Trade Pact), `CommerceRating`
    (Additive, Base — Economy SE / faction bonuses in the commerce tech ratio; social-rating
    expansions live on base effects), `CommerceEnergyBonus` (Additive, Base — flat add per
    commerce transaction; Planetary Governor). Consumed by `CommerceCalculator` during
    `ResourceCollection`; see [economy-system.md](economy-system.md).
