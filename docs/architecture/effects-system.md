...
- **Values**:
  ...
  - Difficulty stats (see [difficulty-system.md](difficulty-system.md)): `SizeFreeDrones` / `BureaucracyFreeDrones` (Additive drone counts — difficulty is the sole emitter, so the count *is* the value), `TechCostDiff` (Additive ordinal fed to `tech_cost.lua` as `diff`), `BureaucracyDifficulty` (Additive ordinal fed to `pop_composition.json`'s `bureaucracy_limit_formula`), `EcologicalDamage` (RawScaled: seed is the accrued amount).
  - Population modifier: `GrowthRate` (`AddPercent`, base = 100%) — modifies the faction-wide population growth rate. `LastDefenderPopLoss` and `CapturePopLoss` are two independent Additive stats whose baselines come from `base_conquest.json`'s own `effects` array (an `Add` each, injected into every faction's pool like `production.json`'s). Perimeter Defense and Citizen difficulty `MaxClamp` 0 the last-defender one only; nothing in the shipping config modifies capture loss. `CaptureFacilitiesDestroyedMin` and `CaptureFacilitiesDestroyedMaxPercent` are the same shape, so **every numeric tunable in `base_conquest.json` is a modifiable stat** — the file holds no scalars at all, only its effects list and the escape-pod component ids. Because each baseline is an ordinary contribution rather than a hard-coded seed, a mod can *raise* these values, not merely clamp them; vanilla simply ships no emitter besides the baseline for most of them.
...
- **`StatKind_t`**: `Additive` (contributions add onto an empty base; seed `0.0`),
  ...
  baseline, `MoistureTier`'s base tier, `FacilityEnergyUpkeep`'s building upkeep,
  `ScrapRefund`'s formula amount, `LastDefenderPopLoss` / `CapturePopLoss`'s `base_conquest.json`
  amount). A stat is RawScaled only when the resolve site holds a pre-existing number for
  modifiers to act on — a value whose sole emitter *is* the effect list stays `Additive`.
