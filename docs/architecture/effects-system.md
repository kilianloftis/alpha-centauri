...
- **Values** (`ConditionKind`):
  - `TargetTileHas` — the targeted tile has the feature id in `condition.value`, matched via `Tile::HasFeature`. One kind covers terrain classification (`Rocky`), river/fungus, landmark, bonus, and any improvement id — including `Base` (a founded base registers itself as the `Base` improvement). In combat the target is the defender's tile.
