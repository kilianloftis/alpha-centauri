...
- **Components**:
  ...
  - `ScrapRefundCalculator`: Prices player scrap from `production.json` `kinds.<kind>.scrap` (`formula` over listed mineral cost, `refund_type`). Owned by `GameDataContext` beside `HurryProductionCalculator` (same `LuaRuntime`), and borrowed by every `BaseManager`. Buildings ship `floor(minerals / 2)` as `energy_credits`. A kind with no scrap block cannot be scrapped. Secret-project scrap and stockpile kinds are rejected at parse. Combat, probes, raze, and orbital fire destroy without calling this. `BaseManager::QuoteScrapBuilding` / `ScrapBuilding` are the current callers
