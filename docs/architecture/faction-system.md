...
graph TB
    ...
    subgraph "Shared config (GameDataContext)"
        ...
        ScrapPayout[ScrapPayout<br/>free functions]
    ...
    BaseManager -->|borrows| ScrapRefundCalculator
    BaseManager -->|prices via| ScrapPayout
    Faction -->|prices via| ScrapPayout
    Faction -->|borrows| ScrapRefundCalculator
...
- **Two consumers, one question**: `BaseManager::IsCurrentProductionPrototype_` asks it for the
  ...
  the `IsPrototype` condition reads. The same ephemeral production arg stamps matching
  `ProducedAtThisBase` effects onto `Unit::GetProductionGrants` and fires
  `on_unit_produced_effects` (`GrantXp` for train / prototype XP). The latch is what keeps the
  filter a context-free identity predicate: the ledger keeps moving as the faction builds, but
  a unit's prototype status is fixed at construction. Free spawns still unlock the ledger
  without latching.
...
- **Components**:
  ...
  - `HurryProductionCalculator`: Prices hurrying — energy credits bought into the mineral stockpile. Owned by `GameDataContext` beside `TechCostCalculator` (it pairs `ProductionConfig_t` with the shared `LuaRuntime`), and borrowed by every `BaseManager`, so no base or faction has to carry a Lua handle of its own. `BaseManager::QuoteHurry` / `HurryProduction` are the only callers. Each kind in `production.json` `kinds.<kind>.hurry` carries its own formula, `mineral_threshold`, and `below_threshold_multiplier`; a kind with no hurry block cannot be hurried, which is how a mod switches the mechanic off for stockpiles, an item class, or the whole game. Partial payments are priced **marginally** — buying `k` minerals costs what it removes from the finish price — so paying in instalments always totals the quoted price
  - **Scrap** splits into three pieces so that pricing, routing, and paying each have one reason to change:
    - `ScrapConfig.h`: the config vocabulary — `ScrapKindConfig_t` (kind default), `ScrapOverride_t` (partial per-item override), `MergeScrapOverride` (layering rule), and the `IsScrapRefundStat` / `ScrapRefundNeedsBase` predicates. Depends on nothing but `StatId_t`, so `BuildingConfig_t` and `UnitComponentConfig_t` can carry a `scrap` override without pulling in the production subsystem. `ScrapConfigParser` reads both JSON shapes and is shared by the production, building, and unit-component parsers, so the three agree on what a scrap block may contain.
    - `ScrapRefundCalculator`: **prices only.** Reads `production.json` `kinds.<kind>.default_scrap` (`formula` over listed mineral cost, `refund_type` as a `StatId_t` whitelist, `refund_ceiling_percent`). Owned by `GameDataContext` beside `HurryProductionCalculator` (same `LuaRuntime`). Unit components and buildings may override formula and/or refund_type, or set `"formula": null` to deny scrap (Headquarters); `ScrapRefund` StatModifiers then scale the formula amount, and the kind's ceiling clamps last — so an item override can change what a refund is *paid in* but never mint more than the kind allows. A kind with no default_scrap cannot be scrapped.
    - `ScrapPayout` (`PlanScrapPayout` / `CreditScrapRefund`, free functions in the `StockpileConversion` mould): **routes and pays.** `PlanScrapPayout` pairs a `ScrapQuote_t` with the destination base and zeroes a base-destined refund that has nowhere to land, so the quote a player is shown is the amount `CreditScrapRefund` grants — the two cannot drift, because the quote path builds the `ScrapPayout_t` the payout path consumes. `CreditScrapRefund` pays `energy_credits` to the treasury, `minerals` to a production stockpile, `nutrients`/`econ`/`labs`/`psych` to those banks, and `energy` through inefficiency and the faction sliders.
    - Callers: player scrap is on `Faction` in both cases, next to units. Buildings ship `floor(minerals / 2)` as `energy_credits` (`QuoteScrapBuilding` / `ScrapBuilding` for one copy at one base; `QuoteScrapBuildings` / `ScrapBuildings` for every constructed copy of that type across this faction's bases). Units ship the same formula as `minerals` (`QuoteScrapUnit` / `ScrapUnit`), credited to the closest friendly base by `TabletopDiagonalDistance` when the unit stands on this faction's territory, otherwise 0. Supply crawlers override the formula to listed cost. Secret-project scrap and stockpile kinds are rejected at parse. Combat, probes, raze, orbital fire, and mineral-support disband destroy without calling any of this.
  ...
  - `MoodLatch`: The pending→active state machine shared by both mood calculators. `Forecast` (Population stage) warns; `Commit` (Mood stage, after PlayerActions) latches. The split exists so the player is told before they act and the result is decided after.
  - `RiotCalculator`: Tracks drone riot state and emits `will_riot`, `is_rioting`, and `riot_ended`. Two sources keep a riot alive and they expire differently: the **natural condition** (riot weights over the threshold) is recomputed on every `Commit`, while a **forced riot** (probe Incite Drone Riots) lasts a fixed number of Mood commits. A forced riot needs its own lifetime precisely because the action does not change composition — the natural condition cannot sustain it. Also counts consecutive commits, which selects the `riot_tiers` escalation tier.
  - `GrowthCalculator`: Computes the tank capacity a base must fill to grow one population, from `GrowthConfig_t` — `(size + 1) * nutrients_per_pop / (GrowthRate/100)`. Growth/starvation decisions are made in the `BaseGrowth` turn stage (`PopulationManager::ApplyGrowth`), which runs after `BaseProduction`.
  - `GrowthConfigParser`: Loads `config/pop_growth.json` and produces a `GrowthConfig_t` holding `nutrients_per_pop`, `nutrient_intake_per_citizen`, and an `effects` list. The two scalars are required positive integers — a `nutrients_per_pop` of 0 makes the growth threshold identically 0, so every base would grow every turn. `effects` is a closed set of `AllOwnerBases` `StatModifier` baselines: exactly the `starting_size` and `max_base_size` stats, op `Add`, amount > 0, both required. They are collected into the faction pool by `FactionEffectsPool::CollectGrowthEffects_`, so a Hab Complex / Dome raises the cap by stacking its own `max_base_size` modifier rather than by mutating a stored field.
...
- **Responsibilities**:
  ...
  - Evaluate drone riot conditions and manage riot lifecycle via signals:
    - `will_riot`: emitted when a riot becomes *pending* — from `AddPop()`, from `ForceRiot`, or from `ForecastMood()` in the Population stage. Pending applies no gameplay effect; it is the warning the player can still act on.
    - `is_rioting`: emitted by `CommitMood()` in the Mood stage when the base becomes rioting from **either** source — the natural condition or an unexpired forced riot
    - `riot_ended`: emitted by `CommitMood()` when neither source holds any longer and the base was previously rioting
