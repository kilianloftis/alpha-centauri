...
// True when rUnit can refuel here: a territory pad that TileHarbors its domain, or embarked
// on a carrier that carries its domain. Co-located but unembarked air over a full deck does
// not count.
...
// When NeedsAutoReturnToFuel, path to the cheapest reachable friendly harbor pad /
// boardable carrier and assign a MoveOrder. Returns true if an order was set. Does not
// execute the order.
